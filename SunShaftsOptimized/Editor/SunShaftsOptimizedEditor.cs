...
{
	[CustomEditor (typeof(SunShaftsOptimized))]
	class SunShaftsOptimizedEditor : Editor
    {
        ...
        public override void OnInspectorGUI () {
			...
			if ((target as SunShaftsOptimized).GetComponent<Camera>())
				GUILayout.Label("Current camera mode: "+ (target as SunShaftsOptimized).GetComponent<Camera>().depthTextureMode, EditorStyles.miniBoldLabel);
		...
		*/
			...
			if ((target as SunShaftsOptimized).sunTransform && (target as SunShaftsOptimized).GetComponent<Camera>()) {
				if (GUILayout.Button("Center on " + (target as SunShaftsOptimized).GetComponent<Camera>().name)) {
					if (EditorUtility.DisplayDialog ("Move sun shafts source?", "The SunShafts caster named "+ (target as SunShaftsOptimized).sunTransform.name +"\n will be centered along "+(target as SunShafts).GetComponent<Camera>().name+". Are you sure? ", "Please do", "Don't")) {
						Ray ray = (target as SunShaftsOptimized).GetComponent<Camera>().ViewportPointToRay(new Vector3(0.5f,0.5f,0));
						(target as SunShaftsOptimized).sunTransform.position = ray.origin + ray.direction * 500.0f;
						(target as SunShaftsOptimized).sunTransform.LookAt ((target as SunShaftsOptimized).transform);
                    }
                }
            }
